...
/* jwlink: if a segment has no class (""), don't add it to AUTO */
...
/***************************************/
{
    ...
    packstart = NULL;
    seg = NULL;
    DEBUG((DBG_OLD,"AutoGroupSect() enter" ));
...
/********************************************************************/
{
    ...
    DEBUG((DBG_OLD,"AllocGroup(%s) exit, flags=%x", name, group->segflags ));
    return( group );
}
...
/***********************************************/
{
    ...
    DEBUG((DBG_OLD,"GetAutoGroup(%d) enter", abs_seg ));
    if( abs_seg ) {
        grp_list = &AbsGroups;
    } else {
        grp_list = &Groups;
...
// Sort the group list by segments within classes.
{
    ...
    DEBUG((DBG_OLD,"SortGroupList() enter" ));
    NumGroups = 0;
    if( Groups == NULL )
        return;
    ...
    DEBUG((DBG_OLD,"SortGroupList() exit" ));
}
...
{
    DEBUG((DBG_OLD,"SortGroup: seg %s", seg->segname ));
    if( seg->group == NULL )
        return;
    if( seg->info & SEG_ABSOLUTE )
        return;
